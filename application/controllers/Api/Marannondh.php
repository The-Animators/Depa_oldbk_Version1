...
class Marannondh extends REST_Controller{
    ...
    */
    function save_post(){
        ...
        $death_contact  = $this->input->post('death_contact');
        $description    = $this->input->post('description');
        $image          = $this->input->post('photo_file');
        $flag           = false;

        // $rules = [        
        //     ['field' => 'death_contact', 'label' => 'death_contact', 'rules' => 'required']
        // ];
        ...
        $this->form_validation->set_rules('mdate', 'Maran Nondh Date', 'required');
        //$this->form_validation->set_rules('death_contact', 'Contact XD', 'required');
        $this->form_validation->set_rules('description', 'Description', 'required');
        //$this->form_validation->set_rules($rules);
        //$this->form_validation->set_rules('photo_file', 'Image', 'required');
        ...
        if ($this->form_validation->run()) {
            ...
            $config['upload_path']  = $this->config->item('upload_path').'marannondh/'; //path folder
            ...
            if($flag){
                $data = array(
                    ...
                    'date' => $date,
                    'contact' => implode(",", $death_contact),//json_encode($death_contact), 
                    'image' => $image, 
                );
                $result = $this->maran_nondh->save_marannond($data, $id);
    ...
    public function index_get($id = '') {
        ...
        $result = $this->maran_nondh->getRows();
    ...
    public function Marannondh_delete($id){
        // echo 'ID '.$id;exit;
        //check whether post id is not empty
        if($id){
            
            // $this->maran_nondh->_condition['id'] = $id;
            // $delete = $this->maran_nondh->delete(FALSE,$id);
            
            $this->db->where('id', $id);
            $delete = $this->db->delete('maran_nondh');
            
            if($delete){
                //set the response and exit
                $this->response([
                    'status' => TRUE,
                    'message' => 'Maran Nondh has been removed successfully.'
                ], REST_Controller::HTTP_OK);
            }else{
                //set the response and exit
