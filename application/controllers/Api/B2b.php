...
class B2b extends REST_Controller{
    ...
    */
    ...
    public function update_post(){
        ...
        if($status == 2){
            $this->form_validation->set_rules('b2b_cancel_reason', 'Enter B2B Cancel Reason', 'required|trim');
        }
        ...
        if ($this->form_validation->run()) {
            ...
            $master['can_reason']       = $this->post('b2b_cancel_reason');
        ...
        if ($this->form_validation->run()) {
            ...
            if($id == 0){
                ...
                if($_FILES['pdf']['size'] > 0){
                    // echo "Calling upload_web for Video <br>\n";
                    $rtn = $this->upload_web('pdf', 'pdf', 'pdf');
                    if($rtn['status'] == true){
                        $master['pdf'] = $rtn['filename'];
                        $logoupload             = true;
                    }else{
                        $logoupload         = false;
                        $data['status']     = false;
    ...
    public function android_post(){
        ...
        if($id == 0){
            ...
            if(!empty($_FILES['logo']['name'])){
                ...
                $rtn = $this->upload_logo_android('logo', 'jpg|png|jpeg', 'logo', rand(10,100));
                //$temp_name .= $rtn.", ";
                if($rtn['status'] == true){
                    $master['logo']     = $rtn['filename'];
                    $logoupload         = true;
            ...
            if(!empty($_FILES['visitingcard']['name'])){
                
                $rtn = $this->upload_vcard_android('visitingcard', 'jpg|png|jpeg', 'vc', rand(10,100));
                //$temp_name .= $rtn.", ";
            ...
            if(!empty($_FILES['bvideo']['name'])){
                
                $rtn = $this->upload_video_android('bvideo', 'mp4|mov|wmv|flv|avi|webm|mkv', 'video', rand(10,100));
                //$temp_name .= $rtn.", ";
            ...
            if(!empty($_FILES['pdf']['name'])){
                ...
                $rtn = $this->upload_pdf_android('pdf', 'pdf', 'pdf', rand(10,100));
                //$temp_name .= $rtn.", ";
            ...
            if(!empty($_FILES['bimages']['name']) && count(array_filter($_FILES['bimages']['name'])) > 0){ 
                ...
                for($i = 0; $i < $countfiles; $i++){
                    ...
                    $rtn = $this->upload_img_android('file', 'jpg|jpeg|png', 'image', $i);
    ...
    public function upload_img_android($img ='', $type='', $what='', $inc=''){
        
        $extension = pathinfo($_FILES[$img]["name"], PATHINFO_EXTENSION);
        $image_tmp      = $_FILES[$img]['tmp_name'];
        $filename     = "image_".date('Ymdhis').$inc.'.jpg';
    ...
    public function upload_pdf_android($img ='', $type='', $what='', $inc=''){
        
        $extension = pathinfo($_FILES[$img]["name"], PATHINFO_EXTENSION);
        $image_tmp      = $_FILES[$img]['tmp_name'];
        $filename     = "image_".date('Ymdhis').$inc.'.pdf';
    ...
    public function upload_logo_android($img ='', $type='', $what='', $inc=''){
        
        $extension = pathinfo($_FILES[$img]["name"], PATHINFO_EXTENSION);
        
        $image_tmp      = $_FILES[$img]['tmp_name'];
        $filename     = "logo_".date('Ymdhis').$inc.'.jpg';
    ...
    public function upload_vcard_android($img ='', $type='', $what='', $inc=''){
        
        $extension = pathinfo($_FILES[$img]["name"], PATHINFO_EXTENSION);
        
        $image_tmp      = $_FILES[$img]['tmp_name'];
        $filename     = $img."_".date('Ymdhis').$inc.'.jpg';
    ...
    public function upload_video_android($img ='', $type='', $what='', $inc=''){
        
        $extension = pathinfo($_FILES[$img]["name"], PATHINFO_EXTENSION);
        
        $image_tmp      = $_FILES[$img]['tmp_name'];
        $filename     = $img."_".date('Ymdhis').$inc.'.mp4';
