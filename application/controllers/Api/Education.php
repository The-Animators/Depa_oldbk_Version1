    ...
    */
    ...
    public function save_post(){
        ...
        if ($this->form_validation->run()) {
            $master_education['education'] = $education;
            $this->education_model->_condition['education'] = $education;
            $this->education_model->_condition['deleted'] = 0;

            $result = $this->education_model->insert($master_education, $id);
            // print $this->db->last_query();
            // print_r($result);exit;
            $this->response([
                'status'   => $result['status'],
                'validate' => TRUE,
                'message'  => 'Education '.$result['msg'],
    ...
    public function list_get($id = '') {
        ...
        $this->education_model->_condition['deleted'] = 0;
        if($id){
            $this->education_model->_condition['id'] = $id;
        }
        $result = $this->education_model->getRows();
        // print $this->db->last_query();
    ...
    public function education_delete($id){
        ...
        if($id){
            //delete post
            $this->education_model->_condition['id'] = $id;
            $delete = $this->education_model->delete(FALSE,$id);
            // print $this->db->last_query();
            if($delete){
                //set the response and exit
                $this->response([
            ...
            $this->response([
                'status' => FALSE,
                'message' => 'No Education were found.'
            ], REST_Controller::HTTP_NOT_FOUND);
        }
    }
