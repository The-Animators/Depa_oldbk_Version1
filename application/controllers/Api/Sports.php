...
class Sports extends REST_Controller{
    ...
    */
    ...
    public function save_post(){
        ...
        $sports  = trim($this->post('sports'));
        $id       = trim($this->post('id'));
        $this->form_validation->set_rules('sports', 'sports', 'required|trim');
        $this->form_validation->set_error_delimiters('', '');
        $this->form_validation->set_message('required', 'Enter %s');
        if ($this->form_validation->run()) {
            $master_sports['sports']            = $sports;
            $this->sports->_condition['sports'] = $sports;
            $this->sports->_condition['deleted'] = 0;

            $result = $this->sports->insert($master_sports, $id);
            // print $this->db->last_query();
            // print_r($result);exit;
            $this->response([
                'status'   => $result['status'],
                'validate' => TRUE,
                'message'  => 'sports '.$result['msg'],
    ...
    public function list_get($id = '') {
        ...
        $this->sports->_condition['deleted'] = 0;
        if($id){
            $this->sports->_condition['id'] = $id;
        }
        $result = $this->sports->getRows();
        // print $this->db->last_query();
    ...
    public function sports_delete($id){
        // echo 'ID '.$id;exit;
        //check whether post id is not empty
        if($id){
            //delete post
            $this->sports->_condition['id'] = $id;
            $delete = $this->sports->delete(FALSE,$id);
            // print $this->db->last_query();
            if($delete){
                //set the response and exit
                $this->response([
                    'status' => TRUE,
                    'message' => 'Sports has been removed successfully.'
                ], REST_Controller::HTTP_OK);
            }else{
                //set the response and exit
