    ...
    */
    ...
    public function save_post(){
        ...
        if ($this->form_validation->run()) {
            $master_area['area']            = $area;
            $this->area->_condition['area'] = $area;
            $this->area->_condition['deleted'] = 0;

            $result = $this->area->insert($master_area, $id);
    ...
    public function list_get($id = '') {
        ...
        $this->area->_condition['deleted'] = 0;
        if($id){
            $this->area->_condition['id'] = $id;
        }
        $result = $this->area->getRows();
        // print $this->db->last_query();
    ...
    public function area_delete($id){
        ...
        if($id){
            //delete post
            $this->area->_condition['id'] = $id;
            $delete = $this->area->delete(FALSE,$id);
            // print $this->db->last_query();
            if($delete){
                //set the response and exit
                $this->response([
