...
class Family_model extends MY_Model
{
    ...
    public function familydetails($id){
        ...
        if(!empty($family)){
            ...
            $this->db->select('fm.id, fm.member_no, CONCAT_WS(" ",fm.first_name, fm.second_name, fm.third_name) as fullname, fm.contact, fm.email, fm.image, fm.relation_id, fm.dod,fm.live_type, fm.delete_reason as reason');
    ...
    public function familydetailsByNo($id){
        //Fetch Family Details
        $rtn = array();
        $this->db->select('id, family_id, member_no, CONCAT_WS(" ",first_name, second_name, third_name) as fullname, relation_id, contact, live_type ');
        $this->db->from('family_member');
    ...
    public function addHead($family_master, $memberdata, $family_marriage = array(), $memberid=0, $family_id=0, $familyNo=''){
        ...
        if($family_id == 0){
            ...
            $family_no  = "DPF".str_pad($familyId, 4, '0', STR_PAD_LEFT);
        ...
        if(count($family_marriage) > 0){
            ...
            $family_marriage['family_id'] = $familyId;
