...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/surname/surname/"+id,
        dataType: 'json',
        success : function(resData){
            ...
            if(status === true){
                ...
                $("#Modal #id").val(0);
                $("#Modal").modal('hide');
                swal("surname Successfully Deleted! ", {
                    icon: "success",
                    timer: 2000,
                });
            }else if(status == false){
                // $.toaster(message, 'Error', 'danger');
                swal("Eroror On Deleteing Surname! ", {
                    icon: "danger",
                    timer: 2000,
                });
                return false;
...
$(document).on("click", "#openmodal" , function(e) {
    $("#Modal").modal('show');
    $("#defaultModalLabel").html('New Surname');
    $("#save").text('SAVE surname');
});
...
function calllist(){
    // alert(base_url+'Api/surname/list');
       ...
       "destroy": true,
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['surname']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/surname/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #surname").val(data[0].surname);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
