...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/sports/sports/"+id,
        dataType: 'json',
...
function calllist(){
    // alert(base_url+'Api/sports/list');
       ...
       "destroy": true,
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['sports']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/sports/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #sports").val(data[0].sports);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
