'use strict';
var modaltitle = 'Donation Category';
...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/donationcategory/donationcategory/"+id,
        dataType: 'json',
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/donationcategory/save",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    ...
    var table = $('#myTable').DataTable({
       ...
       "destroy": true,
        "ajax": ({
            "type": "GET",
            "url": base_url+'Api/donationcategory/list',
        }),
        columns: [
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['donationcategory']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/donationcategory/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #donationcategory").val(data[0].donationcategory);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
