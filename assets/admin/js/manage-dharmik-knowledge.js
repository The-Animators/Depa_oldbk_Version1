'use strict';
var modaltitle = 'Dharmik Knowledge';
...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/dharmikknowledge/dharmikknowledge/"+id,
        dataType: 'json',
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/dharmikknowledge/save",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    // alert(base_url+'Api/dharmikknowledge/list');
    var table = $('#myTable').DataTable({
       ...
       "destroy": true,
        "ajax": ({
            "type": "GET",
            "url": base_url+'Api/dharmikknowledge/list',
        }),
        columns: [
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['dharmikknowledge']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/dharmikknowledge/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #dharmikknowledge").val(data[0].dharmikknowledge);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
