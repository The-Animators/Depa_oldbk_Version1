'use strict';
var modaltitle = 'Business Category';
...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/business/category/"+id,
        dataType: 'json',
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/business/savecategory",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    ...
    var table = table = $('#myTable').DataTable({
        ...
        'ajax': {
            "type": "GET",
            "url": base_url + 'Api/business/category',
        },
    ...
    $.ajax({
        ...
        success : function(resData){
            ...
            if(status === true){
                ...
                $("#Modal #category").val(data[0].category);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
