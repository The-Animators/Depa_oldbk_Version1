...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/occupation/occupation/"+id,
        dataType: 'json',
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/occupation/save",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    // alert(base_url+'Api/occupation/list');
    var table = $('#myTable').DataTable({
       ...
       "destroy": true,
        "ajax": ({
            "type": "GET",
            "url": base_url+'Api/occupation/list',
        }),
        columns: [
            { data: "occupation" },
            { data: "occupation" },
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['occupation']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/occupation/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #occupation").val(data[0].occupation);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
