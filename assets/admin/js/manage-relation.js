...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/relation/relation/"+id,
        dataType: 'json',
        success : function(resData){
            ...
            if(status === true){
                ...
                $("#Modal #id").val(0);
                $("#Modal").modal('hide');
                swal("Relation Successfully Deleted! ", {
                    icon: "success",
                    timer: 2000,
                });
            }else if(status == false){
                // $.toaster(message, 'Error', 'danger');
                swal("Eroror On Deleteing Relation! ", {
                    icon: "danger",
                    timer: 2000,
                });
                return false;
...
$(document).on("click", "#openmodal" , function(e) {
    $("#Modal").modal('show');
    $("#defaultModalLabel").html('New relation');
    $("#save").text('SAVE relation');
});
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/relation/save",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    // alert(base_url+'Api/relation/list');
    var table = $('#myTable').DataTable({
       ...
       "destroy": true,
        "ajax": ({
            "type": "GET",
            "url": base_url+'Api/relation/list',
        }),
        columns: [
            { data: "relation" },
            { data: "relation" },
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['relation']);
...
$(document).on("click", ".getdetail" , function(e) {
    ...
    $.ajax({
        type    : "GET",
        url     : base_url+"Api/relation/list/"+id,
        dataType: 'json',
        success : function(resData){
            // console.log(resData);
            ...
            if(status === true){
                ...
                $("#Modal #relation").val(data[0].relation);
                $("#Modal").modal('show');
            }else if(status == false){
                $.toaster(message, 'Error', 'danger');
                return false;
