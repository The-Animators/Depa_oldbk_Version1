...
function confirmdelete(id){
    $.ajax({
        type    : "DELETE",
        url     : base_url+"Api/bloodgroup/bloodgroup/"+id,
        dataType: 'json',
        success : function(resData){
            ...
            if(status === true){
                ...
                $("#Modal #id").val(0);
                $("#Modal").modal('hide');
                swal("bloodgroup Successfully Deleted! ", {
                    icon: "success",
                    timer: 2000,
                });
            }else if(status == false){
                // $.toaster(message, 'Error', 'danger');
                swal("Eroror On Deleteing bloodgroup! ", {
                    icon: "danger",
                    timer: 2000,
                });
                return false;
...
$(document).on("click", "#openmodal" , function(e) {
    $("#Modal").modal('show');
    $("#defaultModalLabel").html('New bloodgroup');
    $("#save").text('SAVE bloodgroup');
});
...
$(document).on("click", "#save" , function(e) {
    ...
    $.ajax({
        type    : "POST",
        url     : base_url+"Api/bloodgroup/save",
        data    : formdata,
        dataType: 'json',
        success : function(resData){
...
function calllist(){
    ...
    var table = $('#myTable').DataTable({
       ...
       "destroy": true,
        "ajax": ({
            "type": "GET",
            "url": base_url+'Api/bloodgroup/list',
        }),
        columns: [
            { data: "bloodgroup" },
            { data: "bloodgroup" },
        ...
        "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            let action = `
                    ...
                    `;
            $('td:eq(0)', nRow).html(aData['bloodgroup']);
